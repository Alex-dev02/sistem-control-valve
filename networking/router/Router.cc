...
void Router::addPath(std::string &path, std::function<std::string(std::string)> handler) {
    routes.emplace(path, handler);
}

std::function<std::string(std::string)> Router::getPathHandler(std::string &path) {
